...
def build_live_candles_from_csv(df: pd.DataFrame,*,symbol: str,timeframe_seconds: int,) -> list[LiveCandle]:
...
def fetch_history_range(*,broker: Broker,symbol: str,resolution: str,range_from_epoch: int,range_to_epoch: int,) -> pd.DataFrame:
    if range_from_epoch > range_to_epoch:
        return pd.DataFrame()

    return broker.get_history_chunked_epoch(symbol=symbol,resolution=resolution,range_from_epoch=range_from_epoch,
                                            range_to_epoch=range_to_epoch,cont_flag="1",include_live_candle=False,request_delay=0.34,)


def sync_csv_from_history(*,broker: Broker,symbol: str,resolution: str,timeframe_seconds: int,csv_file: str,
                          default_lookback_seconds: int,) -> pd.DataFrame:
...
def seed_manager_from_csv(*,manager: CandleManager,csv_df: pd.DataFrame,symbol: str,timeframe_seconds: int,) -> int:
...
def backfill_gap_to_csv(*,broker: Broker,symbol: str,resolution: str,timeframe_seconds: int,csv_file: str,last_bucket: int,
                        next_live_bucket: int,) -> pd.DataFrame:
...
def main() -> None:
    ...
    candle_5m = CandleManager(timeframe_seconds=300)
    candle_5s.downstream_managers.append(candle_1m)
    candle_1m.downstream_managers.append(candle_5m)
    ...
    csv_5m = sync_csv_from_history(
                                    broker=broker,
                                    symbol=SYMBOL,
                                    resolution="5",
                                    timeframe_seconds=300,
                                    csv_file="candles_5m.csv",
                                    default_lookback_seconds=4*60 * 60,
                                )


    seeded_5s = seed_manager_from_csv(manager=candle_5s,csv_df=csv_5s,symbol=SYMBOL,timeframe_seconds=5,)
    seeded_1m = seed_manager_from_csv(manager=candle_1m,csv_df=csv_1m,symbol=SYMBOL,timeframe_seconds=60,)
    seeded_5m = seed_manager_from_csv(manager=candle_5m,csv_df=csv_5m,symbol=SYMBOL,timeframe_seconds=300,)

    print(f"SEEDED 5S FROM CSV: {seeded_5s}", flush=True)
    print(f"SEEDED 1M FROM CSV: {seeded_1m}", flush=True)
    print(f"SEEDED 5M FROM CSV: {seeded_5m}", flush=True)

    startup_epoch = int(time.time())
    candle_5s.set_startup_epoch(startup_epoch)
    candle_1m.set_startup_epoch(startup_epoch)
    candle_5m.set_startup_epoch(startup_epoch)

    candle_runner = CandleRunner(tick_queue=stream.tick_queue,candle_managers={"5s": candle_5s,},poll_interval=0.05,)
    candle_runner.start()

    print("STARTING MARKET STREAM", flush=True)
    print("SYMBOL:", SYMBOL, flush=True)
    print("TIMEFRAMES: 5s, 1m, 5m", flush=True)
    ...
    last_written_5m = get_last_csv_bucket("candles_5m.csv", SYMBOL)
    ...
    try:
        while True:
            ...
            # candles_1m = candle_runner.pop_closed_candles()
            # main.py
            ...
            for candle in candles_5m:
                ...
                if last_written_5m is not None and candle.bucket_epoch > last_written_5m + 300:
                    gap_df = backfill_gap_to_csv(
                        ...
                        timeframe_seconds=300,
                        csv_file="candles_5m.csv",
                        last_bucket=last_written_5m,
                        next_live_bucket=candle.bucket_epoch,
                    )
                    if not gap_df.empty:
                        last_written_5m = int(gap_df.iloc[-1]["timestamp"])

                if last_written_5m is None or candle.bucket_epoch > last_written_5m:
                    append_candle_to_csv("candles_5m.csv", candle)
                    last_written_5m = candle.bucket_epoch
