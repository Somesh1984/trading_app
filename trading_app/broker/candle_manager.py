...
class CandleManager:
    ...
    - timeframe bucket me candles banana
    - startup running bucket ko partial treat karna
    - closed candles ko output queue me dena
    """
    ...
    def process_pending_tick_queue(self, tick_queue: Queue[dict[str, Any]]) -> None:
        while True:
            try:
                message = tick_queue.get_nowait()
            except Empty:
                break

            self.process_tick_message(message)
